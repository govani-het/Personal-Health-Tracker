...
def authenticate_user(request):
    if request.method == 'POST':
        try:
            user = services.authenticate_user(request)
            ...
            if user:
                ...
                response.set_cookie(os.getenv('ACCESSTOKEN'), value=jwt.encode({
                    ...
                    'exp': datetime.now(UTC) + timedelta(minutes=int(os.getenv('ACCESS_TOKEN_EXP')))
                },
                    SECRET_KEY,
                ...
                refresh = jwt.encode({
                    'user_id': user.user_id,
                    'exp': datetime.now(UTC) + timedelta(hours=int(os.getenv('REFRESH_TOKEN_EXP')))
        ...
        def decorator(request):
            ...
            try:
                ...
                if access_token is None:
                    ...
                    user_data = UserData.objects.get(user_id=data['user_id'])
...
def update_password(request):
    try:
        ...
        if data is not None:
            services.update_password(request, data)
